<div class="card mb-6">
    <div class="card-body">
        ...
        <p class="flex justify-between items-center mb-2">
            ...
            @php $badgeLabel = $customer->status ? __('Active') : __('Inactive');@endphp
            <x-badge :label="$badgeLabel" classes="badge-xs" />
        </p>

        <p class="flex justify-between items-center mb-2">
            <span class="text-xs font-semibold">{{ __('Total Projects') }}</span>
            <x-badge :label="count($customer->projects)" classes="badge-xs" />
        </p>

        <p class="flex justify-between items-center mb-2">
            <span class="text-xs font-semibold">{{ __('Customer Since') }}</span>
            <span class="text-xs">{{ $customer->created_at->format('d.m.Y') }}</span>
        </p>
    </div>
</div>